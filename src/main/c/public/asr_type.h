...
{
    Int32 vad_type;                     // 0: no vad, 1: vad_mode_1(RNN), 2: vad_mode_2(pitch)
    Float32 vad_threshold;              // default: 0.5(only for vad_mode_2) 
...
typedef struct UnivoiceSessionCallback_S
{
    ...
    int(* onFeatureData)(void* pInst, signed char* data, int size);

    // Save acoustic stream format data
    int(* onAcousticData)(void* pInst, signed char* data, int size);

    // Asr Vad Event
    int(* onRecogVadEvent)(void* pInst, E_Vad_Event vadEventType);
